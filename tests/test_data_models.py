...
def test_slack_ingest(tmp_path) -> None:
    ...
    export.mkdir()
    (export / "0001.json").write_text(
        '[{"ts": "1.0", "text": "Hello", "user": "alice"}]', encoding="utf-8"
    )
...
def test_load_docs(tmp_path) -> None:
    ...
    (docs_dir / "001.json").write_text(
        json.dumps(
            {"time_ms": 3000, "doc_id": "DOC-7", "title": "Policy", "body": "Content"}
        ),
        encoding="utf-8",
    )
...
def test_load_tickets(tmp_path) -> None:
    ...
    (ticket_dir / "001.json").write_text(
        json.dumps(
            {"time_ms": 4000, "id": "TCK-1", "status": "closed", "note": "Resolved"}
        ),
        encoding="utf-8",
    )
...
def test_vei_pack_slack_cli(tmp_path) -> None:
    ...
    pack_slack(
        export_path=str(export), channel="#general", actor="alice", output=str(output)
    )
