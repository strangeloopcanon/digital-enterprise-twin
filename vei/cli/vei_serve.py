...
def serve(
    ...
    seed: int = typer.Option(
        None, help="Deterministic seed (default VEI_SEED or 42042)"
    ),
    artifacts_dir: str = typer.Option(
        None, help="Artifacts dir for trace.jsonl (default VEI_ARTIFACTS_DIR)"
    ),
    alias_packs: str = typer.Option(
        "xero", help="CSV of ERP alias packs: xero,netsuite,dynamics,quickbooks"
    ),
    crm_alias_packs: str = typer.Option(
        "hubspot", help="CSV of CRM alias packs: hubspot,salesforce"
    ),
    erp_error_rate: float = typer.Option(
        0.0, help="Deterministic ERP error rate (0.0..1.0). No latency injected."
    ),
    crm_error_rate: float = typer.Option(
        0.0, help="Deterministic CRM error rate (0.0..1.0). No latency injected."
    ),
) -> None:
